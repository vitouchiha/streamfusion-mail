  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let i = 0; i < w * bpp; i++) {
      ...
      if (ft === 0) v = b;
        ...
        const pa = Math.abs(p - c2), pb = Math.abs(a - c2), pc = Math.abs(a + p - 2 * c2);
        v = b + (pa <= pb && pa <= pc ? a : pb <= pc ? p : c2);
      }
      cur[i] = v & 0xFF;
    }
    prev = cur;
    for (let x = 0; x < w; x++) {
      const r = cur[x * bpp];
      const g = ch >= 3 ? cur[x * bpp + 1] : r;
      const bv = ch >= 3 ? cur[x * bpp + 2] : r;
...
async function main() {
  ...
  if (!answer) { console.log('Run: node _test_manual_captcha.js <digits>'); return; }
