...
app.get(HLS_PROXY_PATH, async (req, res) => {
  ...
  try {
    ...
    // For serversicuro.cc: tokens are IP-locked.  When the stream was extracted
    // by browser+wsProxy the token is locked to the wsProxy IP → use that proxy.
    // Only fall back to CF Worker when there's no per-stream proxyUrl.
...
// NOTE: warm-uprot cron has been moved to src/cron/cron_manager.js
// Route is auto-mounted via cronManager.mountRoutes(app) → GET /api/cron/warm-uprot
...
app.get('/api/system/status', requireDebugAuth, (req, res) => {
  ...
  try {
    const cronManager2 = require('./src/cron/cron_manager');
    report.cron = cronManager2.getStatus();
