...
async function test(){
  ...
  if(!imgMatch){console.log('No captcha image! Body:',html.substring(0,500));return;}
  const cap=decodePng(Buffer.from(imgMatch[1],'base64'));
  const answer=ocrDigits(cap.w,cap.h,cap.px);
  console.log('OCR answer:',answer);
  
  // POST - submit answer
  const r2=await fetch(safegoUrl,{method:'POST',headers:{'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36','Content-Type':'application/x-www-form-urlencoded','Cookie':'PHPSESSID='+sessid,'Origin':'https://safego.cc','Referer':safegoUrl},body:'captch5='+answer,redirect:'manual'});
  console.log('POST status:',r2.status);
  console.log('POST Location:',r2.headers.get('location'));
  console.log('POST Set-Cookie:',r2.headers.getSetCookie());
  const body=await r2.text();
  console.log('\n===FULL BODY ('+body.length+')===');
  console.log(body);
  console.log('===END===');
  
  // Parse <a href>
  const hrefMatch=body.match(/<a\s+href=["']([^"']*)/i);
  console.log('\nExtracted href:',hrefMatch?hrefMatch[1]:'NONE');
}
